...
#
# https://github.com/apple/pfl-research/blob/main/benchmarks/model/pytorch/cnn.py
# Copyright © 2023-2024 Apple Inc.
# Licensed under the Apache License, Version 2.0 (the "License").

# Disable some pylints to not have to modify the copied code.
# pylint: disable=missing-function-docstring,expression-not-assigned,no-else-return
...
from flair.metrics import AveragedPrecision
from flair.module_modification import (
...
def multi_label_cnn(
    ...
    """Init CNN used for multi-label classification task.
    ...
    class MultiLabelCNN(nn.Module):
        ...
        def metrics(self, inputs, targets, eval=True):
            ...
            return {
                ...
                "micro loss": (summed_loss.sum().item(), num_predictions.sum()),
                "micro accuracy": (correct.sum().item(), num_predictions.sum()),
                "micro precision": (tps_sum, tps_sum + fps_sum),
                "micro recall": (tps_sum, tps_sum + fns_sum),
                "micro AP": AveragedPrecision(
                    y_true=targets.cpu().numpy(),
                    y_pred=scores.cpu().numpy(),
                    multi_label=False,
                ),
                # Macro metrics: averaged over classes
                "macro loss": (summed_loss.cpu().numpy(), num_predictions),
                "macro accuracy": (correct.cpu().numpy(), num_predictions),
                "macro precision": (
                    tps.cpu().numpy(),
    ...
    return MultiLabelCNN(
        model_type, num_outputs, channel_mean, channel_stddevs, pretrained
    )
