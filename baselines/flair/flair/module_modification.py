...
from flair.layer import FrozenBatchNorm1D, FrozenBatchNorm2D, FrozenBatchNorm3D
...
def _replace_child(
    ...
    Convert a sub-module to a new module given a helper function, the root module and a
    string representing the name of the submodule to be replaced.
    """
...
def replace_all_modules(
    ...
    Convert all the submodules (of root) that have the same type as target_class, given
    a converter, a module root, and a target class type.
    """
    # base case
...
def _batchnorm_to_groupnorm(module: nn.modules.batchnorm._BatchNorm) -> nn.Module:
    """Convert a BatchNorm ``module`` to GroupNorm module. This is a helper function.
...
def convert_batchnorm_modules(
    ...
    Convert all BatchNorm modules to another module (defaults to GroupNorm) that is
    privacy compliant.
...
def freeze_batchnorm_modules(model: nn.Module):
    ...
    Convert all BatchNorm modules to FrozenBatchNorm modules where the stats are
